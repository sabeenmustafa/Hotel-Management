...
def click_checkout():
    ...
    if f_name == '' or l_name == '' or room_no == '':
        messagebox.showwarning("Warning", "Enter valid data")
    else:
         text.delete('1.0', END)
         mydb = mysql.connector.connect(host='localhost', user='root', password='root123', database='hotel', port =8880)
         cur = mydb.cursor()
...
lname_entry = Entry(top_frame, textvar=L_name, bd=5, bg="#FFFAFA", fg='black', width=20, font=('Arial', 15))
...
# Proceed Button
proceed_button = Button(root, text="CHECK OUT", width=10, bg="#7B68EE", fg='White', font=('ARIAL BLACK', 20),
                        relief=RAISED, command=click_checkout)
